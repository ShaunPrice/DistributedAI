...
## Contextual help and support — 8 September 2026

The updated suite passed **269 Docker tests** against a disposable PostgreSQL 17 database on a separate network, with no access to the installed database. Host checks passed 261 tests with eight PostgreSQL opt-in skips; Ruff, four Lambda adapter tests and the unchanged dependency-license inventory check also passed. Support tests cover installation defaults, scoped administration, encrypted ticket/reply storage, role revocation, cross-organisation ticket isolation, injection screening, MCP bindings, duplicate requests and concurrent quotas across two database clients.

Browser checks in a fictional installation verified sign-in help, internal ticket reading and replies, eligible support assignment controls, user-reviewed AI context, organisation-managed external routing and solution-admin configuration persistence. The added navigation exposed a short-screen sign-out layout issue, which was corrected and retested. Two actual screenshots document the support workspace and solution configuration. The installed split web/API service passed authenticated MCP support-tool discovery, help, routing and AI-context checks, plus a read-only platform configuration check; its real support defaults were left unchanged.

External support currently means a configured HTTPS portal opened by the user: no provider API submission, email notification or status synchronisation is implemented. AI support uses the caller's MCP client or an explicitly reviewed copied brief; no inference service or provider credentials are hosted. No real external ticket or LLM-provider interaction was performed. Fixed ticket/reply/storage caps are enforced; automatic retention purge and scoped support export remain unimplemented. Support tables are included in whole-database backups, but existing scoped memory archives exclude them. This increment has not undergone an independent penetration test.
