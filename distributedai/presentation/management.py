...
def management_app(store, settings):
    ...
    async def identity(request):
        try:
            ...
            if len(encrypted) > 16384:
                return None
            if await asyncio.to_thread(store.sessions.is_revoked, encrypted):
                return None
            ...
            if session["kind"] == "oidc":
                ...
                if settings.login_subjects.get(session["sub"]) != session["principal_id"]:
                    return None
                actor = await asyncio.to_thread(store.resolve_principal, session["principal_id"])
                if actor:
                    request.scope["security_actor"] = actor.id
                return actor
        ...
        actor = await asyncio.to_thread(store.resolve_principal, access.client_id) if access else None
        if actor:
            request.scope["security_actor"] = actor.id
        return actor
    ...
    async def login(request):
        ...
        request.scope["security_actor"] = access.client_id
    ...
    async def logout(request):
        if not csrf(request):
            return JSONResponse({"error": "Request origin rejected"}, 403)
        await asyncio.to_thread(store.sessions.revoke, request.cookies.get(COOKIE, ""), TTL)
