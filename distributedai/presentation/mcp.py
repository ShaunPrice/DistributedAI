...
class Limits:
    ...
    Uses the peer resolved by the server's explicit trusted-proxy policy.
    Invalid-token churn shares the same IP bucket.
    An internet edge should also impose a deployment-wide rate and connection budget.
    """
    def __init__(self, app, rpm=300, body_timeout=15):
    ...
    async def __call__(self, scope, receive, send):
        ...
        keys = ["ip:" + peer]
        if token:
            keys.append("token:" + hashlib.sha256(token).hexdigest())
        login = scope.get("path", "").rstrip("/") in {"/manage/login", "/platform/login"}
        if login:
            keys.append("login:" + peer)
        for key in keys:
            started, count = self.buckets.pop(key, (now, 0))
            if now - started >= 60:
                started, count = now, 0
            self.buckets[key] = (started, count + 1)
            budget = min(self.rpm, 10) if key.startswith("login:") else self.rpm
            if count >= budget:
...
def create_server(store: WorkspaceApplication, settings: Settings) -> FastMCP:
